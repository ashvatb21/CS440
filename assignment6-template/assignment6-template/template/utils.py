...
def hyperparameters():
    ...
    # parser.add_argument('--epsilon', type=float, default=0.05, help='exploration parameter') 0.5
    ...
    #                                                               'model should be able to predict future rewards') 0.95
    ...
    # parser.add_argument('--lr', type=float, default=1e-4, help='learning rate') 0.1
...
def rollout(env: gym.Env, policies: policies.QPolicy, episodes: int, epsilon: float, render: bool = False):
    ...
    for _ in range(episodes):
        ...
        while not done:
            ...
            action = np.random.choice(a=[0, 1], size=1, p=pi)[0]
            ### Your code ends here.
